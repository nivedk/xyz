...
//Why don't we have extern volatile struct.... because struct is defined in peripherals.h
volatile struct state Current_state;
unsigned char write_data=0x4C;// OBC, Torquer, Magmeter//01001100
...
int main(void){
  ...
  configure_torquer();
  DDRA=0xF0;
  //struct is defined in peripherals.h
  ...
  transmit_string_UART0("Code");
    ...
    ///* Switch on Global interrupts - This is an atmega defined function to switch on global interrupts
    sei();
  ...
  while (1)
  {
  	  //not clear with what this function does
	  timer_reset_two_sec();
	  //wdt_enable(WDTO_2S);
	  //wdt_reset();
	  //mode is an unsigned integer defined in peripherals.h that specifies the mode of the satellite
	  if (Mode == DETUMBLING) {
	   write_data|=(1<<6)|(1<<3)|(1<<2);
	}
	  if (Mode == NOMINAL){
	  	write_data|=(1<<6)|(1<<3)|(1<<2)|(1<<7);
	  }
	  
	  //how is PB5 being used as slave select/
  PORTB |= (1<<PB5); //Set slave select of ADC =1
	...
	//not sure what these do - they only seem to be described in debug.elf
	wdt_enable(WDTO_2S);
	wdt_reset();
	
//PortA is just for debugging the code - lighting up LEDs
PORTA = 0xAA;
